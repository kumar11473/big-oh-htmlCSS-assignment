...
function renderFooter() {
    let page_numbers_to_show = Math.ceil(apiInstance.total_number_of_fetched_data / apiInstance.offset);
    ...
    function createPages(current_page) {
        ...
        for (let i = 1; i <= page_numbers_to_show; i++) {
            // if (current_page === i) {
            //     ul.innerHTML += `<li><a href="#" class="page_number " data-page="${i}">${i}</a></li>`;
            // } else {
            //     ul.innerHTML += `<li><a href="#" class="page_number" data-page="${i}">${i}</a></li>`;
            // }

            ul.innerHTML += `<li><a href="#" class="page_number ${current_page === i ? 'active_page' : ''}" data-page="${i}">${i}</a></li>`;
        }
